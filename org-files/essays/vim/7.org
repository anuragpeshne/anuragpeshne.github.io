...
* My Vim
  In [[./6.org][last part]], we changed a lot of options, like setting up the line
...
** Leader Key
   You might have preference for certain key combinations which might used by
   Vim or other plugin you installed. If you mapped that combination, which conflicts
   with other mapping, your mapping will simply shadow the previous binding. You
   may not want this.
   ...
   You can map ~<leader>yy~ to map your personal way of yanking a line, or two, and
...
* One more thing
  Let's look at couple of modes more:
  ...
  2. *Replace mode*: Hitting ~R~ switches Vim to a altogether different mode, in which
     ...
     - ~r~: replace, it's like putting momentarily into replace mode. It replaces
       ...
       - ~<shift>-j~: works as complementary to above action. It joins lines. Takes
